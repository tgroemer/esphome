...
void EPaper2P9InBWR::update_full_() {
  ...
  for (uint32_t i = 0; i < buf_half_len; i++) {
    ESP_LOGD(TAG, "Updating bw byte %u %s", i, std::bitset<8>(this->buffer_[i]).to_string().c_str());
    this->data(this->buffer_[i]);
  ...
  for (uint32_t i = buf_half_len; i < buf_len; i++) {
    ESP_LOGD(TAG, "Updating r byte %u %s", i, std::bitset<8>(this->buffer_[i]).to_string().c_str());
    this->data(this->buffer_[i]);
